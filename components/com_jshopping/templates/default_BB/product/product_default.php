...
<div class="jshop productfull" id="comjshop">
    <div class="container">
        <form name="product" method="post" action="<?php print $this->action ?>" enctype="multipart/form-data"
            ...
            <div class="row-fluid jshop product__info-row">
                <div class="span4 image_middle product__info-img">
                    ...
                    <?php if (count($this->videos)) { ?>
                        <?php foreach ($this->videos as $k => $video) { ?>
                            <?php if ($video->video_code) { ?>
                                <div style="display:none" class="video_full"
                                     id="hide_video_<?php print $k ?>"><?php echo $video->video_code ?></div>
                            <?php } else { ?>
                                <a style="display:none" class="video_full" id="hide_video_<?php print $k ?>"
                                   href=""></a>
                            <?php } ?>
                        <?php } ?>
                    <?php } ?>
                    ...
                    <span id='list_product_image_middle'>
                        ...
                        <div class="slider-product">
                            <?php foreach ($this->images as $k => $image) { ?>
                                <!--<a class="" id="main_image_full_<?php /*print $image->image_id */ ?>"
                    ...
                    <?php if ($this->config->product_show_manufacturer_logo && $this->product->manufacturer_info->manufacturer_logo != "") { ?>
                        <div class="manufacturer_logo">
                            <a href="<?php print SEFLink('index.php?option=com_jshopping&controller=manufacturer&task=view&manufacturer_id=' . $this->product->product_manufacturer_id, 2); ?>">
                                <img src="<?php print $this->config->image_manufs_live_path . "/" . $this->product->manufacturer_info->manufacturer_logo ?>"
                                     alt="<?php print htmlspecialchars($this->product->manufacturer_info->name); ?>"
                                     title="<?php print htmlspecialchars($this->product->manufacturer_info->name); ?>"
                                     border="0"/>
                            </a>
                        </div>
                    <?php } ?>
                ...
                <div class="product__info">
                    <h1><?php print $this->product->name ?><?php if ($this->config->show_product_code) { ?> <span
                                class="jshop_code_prod">(<?php print _JSHOP_EAN ?>: <span
                                    id="product_code"><?php print $this->product->getEan(); ?></span>)</span><?php } ?>
                    </h1>
                    ...
                    <?php if ($this->product->_display_price) { ?>
                        <div class="prod_price">
                            ...
                            <div class="product__info-price dd">
                            ...
                            <?php foreach ($this->attributes as $attribut) : ?>
                                ...
                                <div class="row-fluid">
                                    <div class="span2 attributes_title">
                                        <span dadata class="attributes_name"><?php print $attribut->attr_name ?>:</span>
                                        <!--                                        <span class="attributes_description">-->
                                        <?php //print $attribut->attr_description; ?><!--</span>-->
                                    </div>
                                    <div class="span10">
                                        <span class="product__info-color-product"
                                              id='block_attr_sel_<?php print $attribut->attr_id ?>'>
                    ...
                    <div class="product__counter-block">
                        ...
                        <!--                                <a href="/dingh" class="basket__btn-next">Добавить в корзину</a>-->
                        <div class="product__btn-in-cart">
                            <input type="submit" value="<?php print _JSHOP_ADD_TO_CART ?>"
                                   onclick="jQuery('#to').val('cart');"/>

                            <?php if ($this->enable_wishlist) { ?>
                                <input type="submit" class="btn button" value="<?php print _JSHOP_ADD_TO_WISHLIST ?>"
            ...
            <div class="tabs__nav tabs-nav">
                <div class="tabs-nav__item is-active" data-tab-name="specifications">Характеристики</div>
                <div class="tabs-nav__item" data-tab-name="equipment">Комплектация</div>
                <div class="tabs-nav__item" data-tab-name="description">Описание</div>
                <div class="tabs-nav__item overview" data-tab-name="overview">Обзор</div>
                <div class="tabs-nav__item rev" data-tab-name="rev">Отзывы</div>
            </div>

            <div class="tabs__content">
                <div class="tab is-active specifications">
                    <?php if (is_array($this->product->extra_field)) { ?>
                        <ul class="unstyled extra_fields">
                            <?php $extra_field_group = "";
                            $flag = 0;
                            foreach ($this->product->extra_field as $extra_field) {
                                if (!empty($extra_field['groupname'])) {
                                    continue;
                                }
                                if ($extra_field_group != $extra_field['groupname']) {
                                    ...
                                    <li class='extra_fields_group'>
                                        <h3><?php print $extra_field_group ?></h3>
                                    </li>
                                <?php } else {
                                    ?>
                                ...
                                <?php } ?>
                                <li class="row-fluid specification-list">

                                    <?php if (strcmp($extra_field_group, '') == 0) { ?>
                                        <span class="span5 extra_fields_name"><?php print $extra_field['name']; ?>:</span>
                                        <span class="span7 extra_fields_value"><?php print $extra_field['value']; ?></span>
                                    <?php } else {
                                        ?>
                ...
                <div class="tab equipment">
                    <?php if (is_array($this->product->extra_field)) { ?>
                        <ul class="unstyled extra_fields equipment-list">
                            <?php $extra_field_group = "";
                            $flag = 0;
                            foreach ($this->product->extra_field as $extra_field) {
                                if (empty($extra_field['groupname'])) {
                ...
                <div class="tab overview">
                    <?php if (($flag == 0) and (strcmp($this->product->description, '') != 0)) { ?>
                        <div id="specs">
                            <div class="jshop_prod_description">
                                <?php print $this->product->description;
                                $flag = 1; ?>
            ...
            <!-- <?php /*if ($this->product->product_url != "") { */ ?>
                <div class="prod_url">
                    <a target="_blank"
                       href="<?php /*print $this->product->product_url; */ ?>"><?php /*print _JSHOP_READ_MORE */ ?></a>
                </div>
            --><?php /*} */ ?>
            ...
            <?php /*if ($this->product->product_price_default > 0 && $this->config->product_list_show_price_default) { */ ?><!--
                <div class="default_price"><?php /*print _JSHOP_DEFAULT_PRICE */ ?>: <span
                            id="pricedefault"><?php /*print formatprice($this->product->product_price_default) */ ?></span>
                </div>
            --><?php /*} */ ?>
        ...
        <?php /*if ($this->config->product_show_button_back) { */ ?><!--
            <div class="button_back">
                <input type="button" class="btn button" value="<?php /*print _JSHOP_BACK; */ ?>"
		...
		<div class="tab-pane <?php if (count($this->related_prod)){echo 'active';} ?>" id="related">
            <?php
                print $this->_tmp_product_html_before_related;
                include(dirname(__FILE__)."/related.php");
            ?>
