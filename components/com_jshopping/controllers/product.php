...
class JshoppingControllerProduct extends JshoppingControllerBase{
    ...
    function display($cachable = false, $urlparams = false){
        ...
        $arrIds = [26, 27, 28, 29, 30];
        $categories = [];
        foreach ($arrIds as $item) {
            array_push($categories, $this->addCategoryWithProducts($item));
        }
        ...
        $view->assign('categories_additional_equipment', $categories);
    ...
    function addCategoryWithProducts($id) {

        // безопасное соединение с базой данных
        $db =&JFactory::getDBO();

        // сам запрос
        $query = "SELECT pivot.category_id, pivot.product_id, products.product_id, category.category_id, category.`name_ru-RU` as category_name, category.`alias_ru-RU` as category_url, products.`alias_ru-RU` as products_url, products.image, products.average_rating, products.`name_ru-RU`, products.min_price 
                    FROM g8jnm_jshopping_products_to_categories as pivot LEFT JOIN g8jnm_jshopping_products as products
                    ON pivot.product_id = products.product_id 
                    LEFT JOIN g8jnm_jshopping_categories as category ON pivot.category_id = category.category_id 
                    WHERE pivot.category_id in (".$id.") limit 3";
        $db->setQuery($query);
