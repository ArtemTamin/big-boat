...
class PlgSystemReReplacerHelperVariables
{
	...
	private function getDateFromFormat($date)
	{
		...
		{
			$date = RLText::dateToDateFormat($date);
		}
	...
	private function replaceThIndDate(&$date, $th = '[TH]')
	{
		...
		preg_match_all('#([0-9]+)' . RLText::pregQuote($th) . '#si', $date, $date_matches, PREG_SET_ORDER);
		...
		foreach ($date_matches as $date_match)
		{
			...
			$date = RLText::strReplaceOnce($date_match['0'], $date_match['1'] . $suffix, $date);
		}
