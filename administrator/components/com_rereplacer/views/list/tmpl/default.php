...
RLFunctions::stylesheet('rereplacer/style.min.css', '7.2.0');
...
{
	echo RLVersions::render('REREPLACER');
}
?>
			...
			<tbody>
				...
				<?php else: ?>
					<?php foreach ($this->list as $i => $item) : ?>
						...
						{
							$case = '<span class="btn btn-micro disabled" rel="tooltip" title="' . JText::_('RL_NOT') . ' ' . JText::_('RR_CASE_SENSITIVE') . '"><span class="icon-cancel"></span></a>';
						}
						if ($item->regex)
						{
						...
						{
							$regex = '<span class="btn btn-micro disabled" rel="tooltip" title="' . JText::_('RL_NOT') . ' ' . JText::_('RR_REGULAR_EXPRESSIONS') . '"><span class="icon-cancel"></span></a>';
						}
						...
						{
							$enable_in_admin = '<span class="btn btn-micro disabled" rel="tooltip" title="' . JText::_('RL_NOT') . ' ' . JText::_('RR_ENABLE_IN_ADMIN') . '"><span class="icon-cancel"></span></a>';
						}
...
echo RLLicenses::render('REREPLACER');

// Copyright
echo RLVersions::getFooter('REREPLACER');
